...
export default function ProductPicker({
  ...
  const { visible, hasMore, remaining, showMore, reset } = useProgressiveReveal(catalog, { initial: 8, increment: 10 })
  ...
  return (
    <div className="flex flex-col gap-4 lg:gap-6">
      ...
      <div className="relative">
        ...
        <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); reset() }}
      ...
      {!query.trim() && <p className="text-xs text-[var(--color-ink-muted)] lg:text-sm">Your products</p>}
      ...
      {loading ? (
        ...
        <p className="py-6 text-center text-sm text-[var(--color-ink-muted)] lg:py-8 lg:text-base">No products match "{query}"</p>
      ) : (
        <>
