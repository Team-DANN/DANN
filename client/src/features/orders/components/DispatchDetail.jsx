...
export default function DispatchDetail({ dispatch, onBack, onMarkPaid }) {
  ...
  return (
    <div className="flex flex-col gap-6 lg:gap-8">
      ...
      <div>
        <h2 className="font-sans text-xl font-bold text-[var(--color-ink)] lg:text-3xl">{retailer?.name}</h2>
      ...
      <div className="flex flex-col gap-2 lg:gap-3">
        {dispatch.items.map((item, i) => (
          <div
            key={i}
            className="flex items-center justify-between rounded-lg border border-[var(--color-border)] bg-[var(--color-paper-light)] px-4 py-2.5 lg:px-6 lg:py-4"
          >
            <span className="text-sm text-[var(--color-ink)] lg:text-base">{productName(item.productId)}</span>
            <span className="font-mono text-sm text-[var(--color-ink-muted)] lg:text-base">×{item.qty}</span>
          </div>
        ))}
      </div>
