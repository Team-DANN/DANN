...
export default function ReceivablesSnapshot({ dispatches }) {
  ...
  if (owing.length === 0) {
    return (
      <p className="rounded-xl border border-[var(--color-border)] bg-[var(--color-paper-light)] px-4 py-3 text-sm text-[var(--color-ink-muted)]">
  ...
  return (
    <div className="flex flex-col gap-2">
      {owing.slice(0, 5).map(({ dispatch, summary }) => {
        ...
        return (
          <Link
            key={dispatch.id}
            to="/orders"
            className="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-paper-light)] px-4 py-2.5 active:bg-[var(--color-paper)]"
          >
            <span className="truncate text-sm font-medium text-[var(--color-ink)]">
              {retailer?.name ?? 'Unknown retailer'}
            </span>
            <span
              className={`flex flex-shrink-0 items-center gap-1 font-mono text-sm font-medium ${
                summary.overdue ? 'text-[var(--color-error)]' : 'text-[var(--color-ink-muted)]'
              }`}
            >
              {summary.overdue && <AlertTriangle size={12} strokeWidth={2} />}
      ...
      {owing.length > 5 && (
        <Link to="/orders" className="text-center text-sm font-medium text-[var(--color-stamp)]">
